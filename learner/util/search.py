...
""" Module containing useful methods and configurations for search experiments. """
...
_EXPERIMENT_DIR = os.path.join(ROOT_DIR, "experiment")

_AUX_DIR = os.path.join(_EXPERIMENT_DIR, "aux_dir")
_PLAN_DIR = os.path.join(_EXPERIMENT_DIR, "plans")
...
def search_cmd(args):
    """Create search command based on args. 
    Mainly use FastDownward.
    """
    cmd = {
        "fd": fd_cmd,
        "pwl": pwl_cmd,
...
def fd_cmd(df, pf, m, search, timeout=TIMEOUT):
    """Create FastDownward search command. 
    
    Args:
        df: Domain PDDL file
        pf: Problem PDDL file
        m: model file
        search: search strategy("gbbfs")
        timeout: search time limit

    Return: 
        command string, SAS plan file
    """
    if search == "gbbfs":
