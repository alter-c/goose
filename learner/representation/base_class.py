...
class Representation(ABC):
    ...
    def convert_to_pyg(self) -> None:
        ...
        self.x = pyg_G.x  # node feature tensor data (N * F)

        if self.n_edge_labels == 1:
            self.edge_index = pyg_G.edge_index  # (2 * E) E: num of edge (double for undirected graph), 2: two points of edge
        else:
            assert self.n_edge_labels > 1
            self.edge_indices = [[] for _ in range(self.n_edge_labels)]  # (2 * E_i) E_i: num of edge labels
